...
export default function ChatPage() {
  ...
  const handleAnalyze = async () => {
    ...
    if (!formData.repoUrls || !formData.projectSummary) {
    ...
    try {
      const githubUrl = formData.repoUrls.split('\n')[0].trim();
      ...
      if (error.response?.data?.detail) {
        ...
        // You could show these errors to the user in the UI
      } else {
        console.error(error);
      }
    } finally {
          ...
          <Card className='floating-card bg-gradient-to-br from-cyan-500/10 via-card/30 to-green-500/10 backdrop-blur-xl border border-cyan-500/20 shadow-2xl shadow-cyan-500/10 hover:shadow-cyan-500/20 transition-all duration-500 hover:scale-[1.02] hover:-translate-y-2'>
            ...
            <CardContent className='space-y-4'>
              <div className='space-y-3'>
                ...
                <DatePicker
                  selected={
                    formData.hackathonStartDate
                      ? new Date(formData.hackathonStartDate)
                      : null
                  }
                ...
                <DatePicker
                  selected={
                    formData.hackathonEndDate
                      ? new Date(formData.hackathonEndDate)
                      : null
                  }
                  onChange={(date: Date | null) =>
                    setFormData((prev) => ({
